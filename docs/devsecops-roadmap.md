...
- Заказчик разворачивает **на Docker, без Kubernetes** → инсталлер = автономный
  **air-gapped Docker-бандл** (образы + `docker-compose` + скрипт установки).
  ⚠️ Kubernetes-кластер ниже — это твоя **ВНУТРЕННЯЯ** инфраструктура для
  сборки / тестов / безопасности / мониторинга и обучения. Он **НЕ едет заказчику.**
  Продукт клиенту — Docker-артефакт (тот же, что мы уже собрали в `DOCKER.md`).
- Ресурсы можно нарастить → полный стек, отдельный build-стенд.
- CD через **Jenkins (CI + сканеры) + ArgoCD (GitOps)** — для твоего внутреннего
  кластера (staging/тесты). Финальная приёмка гоняется по Docker-артефакту (Фаза 12).
...
```
  ...
  └──────────────────────────────────────┴─────────────────┘    │
                                                                 │
  Отдельно: build-стенд собирает air-gapped DOCKER-bundle       │
  (образы + compose + install.sh) → инсталлер заказчику ────────┘
  (заказчик ставит на голом Docker, без k8s)
```
...
Сейчас приложение живёт в `docker-compose`. Для **твоего внутреннего кластера** его
надо описать манифестами, а лучше сразу **Helm-чартом** — это шаблонизируемый пакет,
из которого соберётся staging для тестов/сканирования/мониторинга.

> Две дорожки одного приложения — не путай:
> - **Helm-чарт → k8s** — ТВОЯ внутренняя среда (CI/CD, staging, DAST, мониторинг, обучение).
> - **Docker-бандл → заказчику** (Фаза 12) — то, что реально едет клиенту, без k8s.
>
> Один и тот же образ (проверенный пайплайном) используется в обеих: в k8s его тянет
> ArgoCD из Harbor, в бандл он попадает через `docker save`. Чарт заказчику не нужен.
...
## Фаза 12. Упаковка инсталлера для заказчика (air-gapped Docker-бандл)

Твоя цель-продукт: заказчик **на голом Docker, без Kubernetes** ставит приложение
«одной командой». Решение — автономный офлайн-бандл: сохранённые образы +
`docker-compose` + скрипт установки. Никакого k3s/k8s в артефакте.

Это прямое развитие того, что уже есть в `DOCKER.md` — добавляем офлайн-упаковку.

Идея инсталлера (собирается на build-стенде, в идеале — стадией Jenkins после того,
как образ прошёл все проверки и сканирования):

```
transfermarket-installer/
├── install.sh              # один вход: ставит docker (если нет), грузит образы, поднимает
├── images/
│   └── transfermarket.tar  # твои образы, сохранённые `docker save` (офлайн, без registry)
├── docker-compose.yml      # тот же compose, но image: <локальный тег>, без build:
├── .env.example            # заказчик впишет свои секреты (JWT_SECRET и т.п.)
├── VERSION                 # версия сборки (для апдейтов/поддержки)
└── README.md               # инструкция для заказчика
```

Что делает `install.sh` у заказчика:
1. проверяет/ставит Docker + docker-compose plugin (или использует уже стоящий);
2. импортирует образы из tar: `docker load -i images/transfermarket.tar` — **без
   интернета и без доступа к твоему Harbor** (это и есть «air-gapped»: всё внутри бандла);
3. если нет `.env` — копирует из `.env.example` и просит заполнить секреты;
4. `docker compose up -d`;
5. ждёт healthcheck и выводит URL, где поднялось приложение.

Почему так, а не registry: заказчику не нужен доступ к твоему приватному Harbor и
вообще к сети — всё едет в одном архиве. `docker save`/`docker load` переносят образ
как файл. Это классический паттерн доставки в закрытые/air-gapped контуры.

Как это связано с пайплайном:
- Jenkins собирает и **сканирует** образ (Фазы 5–6) → пушит в Harbor (для твоего
  внутреннего staging через ArgoCD) → **и параллельно** делает `docker save` этого же
  проверенного образа, кладёт в бандл, архивирует бандл как артефакт релиза.
- Так заказчик получает **ровно тот образ, что прошёл SAST/DAST/скан** — immutable artifact.

> Про версии compose в бандле: зафиксируй в `docker-compose.yml` конкретные теги
> образов (не `latest`) и версию формата, совместимую и с V1, и с V2 — чтобы
> поднялось у заказчика с любым Docker. Логи ограничь (`logging: max-size/max-file`),
> чтобы у клиента не повторился твой сюжет с забитым `/var/log`. 😄

**Тестирование артефакта (важно — «тестируй то, что отдаёшь»):**
Финальная приёмка должна гоняться **по Docker-бандлу**, а не только по k8s-staging.
Заведи стадию/отдельную чистую VM, где Jenkins разворачивает собранный `install.sh`
и прогоняет по нему smoke-тест (+ можно ZAP из Фазы 8). Это гарантирует, что «одна
команда» реально работает на чистой машине заказчика.

**📚 Учить:**
- [docker save / docker load (офиц.)](https://docs.docker.com/reference/cli/docker/image/save/)
- [Compose file reference](https://docs.docker.com/reference/compose-file/) ·
  [logging options](https://docs.docker.com/config/containers/logging/configure/)
- Паттерн air-gapped доставки: [Offline installation of Docker images](https://docs.docker.com/engine/install/) +
  концепция «offline bundle» (bundle = images tar + compose + скрипт)
- Продвинуто, для кругозора (как индустрия заворачивает продукты заказчику):
  [Replicated](https://www.replicated.com/) — но тебе достаточно простого bash-инсталлера.

**Контрольная точка:** на чистой Ubuntu-VM **без интернета** `sudo ./install.sh` за
пару минут поднимает рабочее приложение на Docker. Это и есть твой «продукт заказчику».
