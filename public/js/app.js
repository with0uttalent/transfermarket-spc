        ...
        <tbody>
          ${filtered.length ? filtered.map(p=>{
            ...
            <tr class="clickable-row" onclick="navigate('/players/${p.id}')">
              ...
              <td><div class="flex-center gap-2">${avatarEl(p.image_url,p.name)}<div><span class="font-bold">${escHtml(p.name)}</span>${banned?` <span class="badge" style="background:rgba(231,76,60,.18);color:#e74c3c;font-size:10px;padding:2px 6px;border-radius:4px;vertical-align:middle" title="Торговый бан — нельзя продать в этом сезоне">🔒 Бан</span>`:''}${p.on_loan?` <span class="badge" style="background:rgba(52,152,219,.18);color:#3498db;font-size:10px;padding:2px 6px;border-radius:4px;vertical-align:middle" title="Игрок арендован${p.loan_from_team_name?' из '+p.loan_from_team_name:''}">🤝 Аренда</span>`:''}${p.on_loan_out?` <span class="badge" style="display:inline-flex;align-items:center;gap:4px;background:rgba(230,126,34,.18);color:#e67e22;font-size:10px;padding:2px 6px;border-radius:4px;vertical-align:middle" title="В аренде в ${escHtml(p.loan_to_team_name||'')}">${p.loan_to_team_logo?`<img src="${escHtml(p.loan_to_team_logo)}" style="width:14px;height:14px;object-fit:contain;border-radius:2px">`:''}<span>↗ В аренде${p.loan_to_team_name?' в '+escHtml(p.loan_to_team_name):''}</span></span>`:''}</div></div></td>
              <td>${p.flag_emoji||'–'}</td>
              <td>${posBadge(p.position)}</td>
...
function triggerGoalCelebration(side, match, ev) {
  ...
  banner.innerHTML = `
    ${patternHtml}
    <div class="goal-banner-body${scorerName ? ' goal-banner-body--with-scorer' : ''}">
      ...
      <div class="goal-banner-scorer">
        ...
        <span class="goal-banner-scorer-name">${escHtml(scorerName)}</span>
      </div>` : ''}
    </div>
  `;
...
async function renderAdmin(app) {
        ...
        <div style="display:grid;grid-template-columns:1fr 1fr;gap:20px">
          <div>
            ...
            ${[
              ...
              ['tg_channel_player_news','📰 Слухи и новости игроков','Трансферные слухи, скандалы, контракты'],
            ].map(([key, label, hint]) => `
  ...
  GET('/admin/settings').then(s => {
    ...
    for (const [key, def] of [
      ...
      ['tg_channel_coach_news', '0'], ['tg_channel_player_news', '0'],
