...
function generateMatchFullStats(homeStr, awayStr, homeScore, awayScore) {
  ...
  const homePoss = Math.min(70, Math.max(30, rawPoss));

  const homeShots = Math.max(homeScore + 2, homeScore * ri(3, 5) + ri(3, 7));
  const awayShots = Math.max(awayScore + 2, awayScore * ri(3, 5) + ri(3, 7));
  const homeSoT   = Math.min(homeShots, homeScore + ri(1, 4));
  const awaySoT   = Math.min(awayShots, awayScore + ri(1, 4));

  return {
    possession_home: homePoss,
    possession_away: 100 - homePoss,
    shots_home: homeShots,
    shots_away: awayShots,
    shots_on_target_home: homeSoT,
    shots_on_target_away: awaySoT,
    corners_home: ri(2, 9),
    corners_away: ri(2, 9),
    fouls_home: ri(8, 18),
    fouls_away: ri(8, 18),
    offsides_home: ri(0, 5),
    offsides_away: ri(0, 5),
  };
}
...
function simulateMatch(homeTeamId, awayTeamId, homePlayers, awayPlayers) {
  ...
  for (const [pid, s] of Object.entries(stats)) {
    ...
    if (s.goals >= 3) delta += 3.5;
    mvDeltas[pid] = delta;
  }

  const matchStats = generateMatchFullStats(homeStr, awayStr, homeScore, awayScore);

  return { homeScore, awayScore, events, playerStats: stats, mvDeltas, matchStats };
}
